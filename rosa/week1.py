...
# 간단하게 프로그램적으로 구동할 수 있는 것들을 만들어보고싶다.
#  
...
# 데이터가 사라져버리는 휘발성으로 인해 데이터의 별도 장비가 필요함.
# Secondary Memory > 보조 기억 장치
# 위에서 언급한 데이터의 보존을 위한 장치로서 상대적으로 주기억장치에 비해 느리지만, 
# 전원을 종료하더라도 데이터를 보존하고 있기에 사용하는 장치

# 전산 작업의 하드웨어적인 흐름
#   Secondary Memory (데이터 저장 및 보관) 
# > Main Memory (Secondary Memory로부터 데이터를 불러와서 작업진행중인 데이터 보유) 
# > CPU (실제연산처리)
...
birth = input('생일이 지났나요? 지났으면 0, 아니면 -1 : ')
